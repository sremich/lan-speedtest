...
What each figure on the page means, and which ones are worth acting on.

## The headline figures

![The headline figures](images/headline-figures.png)
...
bad on a call regardless of how many megabits it moves. The two small figures
under Latency are the loaded readings, arrowed by direction.
...
loaded remains meaningful even when both are near the resolution floor. The
page says so under the ratings when the readings sit at that floor, rather than
presenting them as precise.

## The step strip

![The step strip during a run](images/progress-strip.png)
...
transfers have not begun.

Above the strip, a line names the stage that is running right now, in that
stage's colour, with the payload it is moving. Its spinner follows the engine's
own running state: pausing stops it, and a finished run hides it rather than
leaving it turning.

Hovering a chevron gives the stage's payload size, its request count, its
position in the profile, and — once it has finished — what it measured:

![A chevron's tooltip](images/progress-strip-tooltip.png)
...
90th percentile marked. Hovering a point gives that individual request:

![A single sample on the download trace](images/trace-tooltip.png)

*Speed, payload size, round trip, and how long the request took. The gap
between "Speed" here and the headline figure is the difference between one
sample and the percentile across all of them.*
...
![The quality ratings](images/quality-ratings.png)
...
rather than showing an empty panel — see
[Troubleshooting](Troubleshooting.md#no-quality-ratings-everything-else-fine),
which is also where the Firefox caveat lives.
...
![The distribution view](images/distribution.png)
...
![A box plot row's tooltip](images/distribution-tooltip.png)

*What the box and whiskers mean, then the minimum, maximum, average, median and
the two quartiles, each named rather than abbreviated, and how many samples it
is drawn from.*

### Why bandwidth is split by transfer size

Each transfer size gets its own row. A small transfer spends most of its time
on round-trip overhead rather than moving data, so its throughput is
legitimately lower — pooling sizes into one distribution would make a perfectly
...
![The raw throughput panel](images/raw-throughput.png)

*The same link, measured twice: 670 Mbps through the engine, 1.01 Gbps across
four parallel streams. Neither is wrong.*

The engine issues one request at a time and reads every response through
`r.text()`, decoding the whole payload into a JavaScript string. That
faithfully represents what a single transfer experiences, and it is bounded by
...
is normal. **That gap is the engine's design showing, not a fault**, and
neither number is "the real one": they measure different things.
...
Widening boxes over time, or outliers appearing where there were none, are
worth investigating before the average moves — which is the point of keeping
[history](History-and-Metrics.md).
